...
* **Язык:** Комментарии и docstrings пишутся на русском языке.
...
try:
    ...
    exit()
```
## 7. Написание тестов (не предгалать - пока не попрошу конкретно написать тесты)
...
* Основной принцип проектирования игровых объектов:
  Объекты должны хранить минимально необходимый набор зависимостей для автономной работы

    Хранить только нужные зависимости 
        EventBus для публикации событий
        GameConfig для параметров баланса
        Context для доступа к системным сервисам
         
    Не хранить зависимости между свойствами 
        Свойства не ссылаются друг на друга напрямую
        Зависимости управляются через события

    Явная инициализация 
        Зависимости передаются через конструктор или методы
        Нет скрытых глобальных переменных
        Легко подменить зависимости для тестирования
         
    Принцип единственной ответственности 
        Каждый объект решает одну задачу
        Сложное поведение - через композицию объектов
        Минимум логики в конструкторах
...
* По возможности следовать принципу "There should be one-- and preferably only one --obvious way to do it"
